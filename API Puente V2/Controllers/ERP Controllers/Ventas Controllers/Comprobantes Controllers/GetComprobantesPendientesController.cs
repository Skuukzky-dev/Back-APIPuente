...
namespace API_Puente_V2.Controllers.ERP_Controllers.Ventas_Controllers.Comprobantes_Controllers
{
    ...
    [ApiController]
    public class GetComprobantesPendientesController : ControllerBase
    {
        // GET: api/<GetComprobantesPendientesController>
        [HttpGet]
        [SwaggerResponse(200, "OK", typeof(cResponseMovimientoDeClientes))]
        public async Task<IActionResult> GetComprobantesPendientes(string fechahasta = "",int clienteID = 0, int pageNumber = (int)APIHelper.pPaginacion.pPageNumber, int pageSize = (int)APIHelper.pPaginacion.pPageSize)
        {
            ...
            try
            {
                ...
                oRespuesta = await GESI.CORE.API.PUENTE.BLL.MovimientosDeClientesMgr.GetComprobantesPendientes(URLBackend,Token,empresaID,sucursalID, Endpoints.strGetComprobantesPendientes,fechahasta,clienteID);
