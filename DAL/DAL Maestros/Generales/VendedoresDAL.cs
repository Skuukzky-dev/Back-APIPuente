    ...
    public class VendedoresDAL
    {
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        oRespuesta = JsonConvert.DeserializeObject<cResponseVendedores?>(respuesta);
        ...
        public static async Task<cResponseVendedor> GetItem(string URLBackend, string Token, int empresaID, int sucursalID, string endpoint, int vendedorID)
        {
            try
            {
                ...
                string cadenaConParametros = "vendedorID="+vendedorID;
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        string respuesta = response.Content.ReadAsStringAsync().Result.ToString();
                        oRespuesta = JsonConvert.DeserializeObject<cResponseVendedor?>(respuesta);
