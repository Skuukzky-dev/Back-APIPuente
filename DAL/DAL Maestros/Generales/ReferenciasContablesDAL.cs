...
{
    public class ReferenciasContablesDAL
    {
        /// <summary>
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        oRespuesta = JsonConvert.DeserializeObject<cResponseReferenciasContables>(respuesta);
        ...
        public static async Task<cResponseReferenciaContable> GetItem(string URLBackend, string Token, int empresaID, int sucursalID, string endpoint, int referenciaContableID )
        {
            ...
            try
            {
                ...
                string cadenaConParametros = "referenciaContableID=" + referenciaContableID;
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        string respuesta = response.Content.ReadAsStringAsync().Result.ToString();
                        oRespuesta = JsonConvert.DeserializeObject<cResponseReferenciaContable>(respuesta);
