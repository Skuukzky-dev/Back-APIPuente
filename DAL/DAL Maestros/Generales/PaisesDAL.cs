...
namespace GESI.CORE.API.PUENTE.DAL
{
    public class PaisesDAL
    {
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        oRespuesta = JsonConvert.DeserializeObject<cResponsePaises?>(respuesta);
        ...
        /// <summary>
        /// Devuelve un pais con su regiones
        /// </summary>
        /// <param name="URLBackend"></param>
        /// <param name="Token"></param>
        /// <param name="endpoint"></param>
        ...
        public static async Task<cResponsePais> GetItem(string URLBackend, string Token, string endpoint, int empresaID, int sucursalID, int paisID)
        {
            try
            {
                ...
                string cadenaConParametros = "paisID="+paisID;
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), URLBackend + endpoint + "?" + cadenaConParametros))
                    {
                        ...
                        string respuesta = response.Content.ReadAsStringAsync().Result.ToString();
                        oRespuesta = JsonConvert.DeserializeObject<cResponsePais?>(respuesta);
